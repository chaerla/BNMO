...
#data = [["id","username","nama","password","role","saldo"],[1,"luffy","luffy","abc","user",3]]
...
def CekKepemilikan(dataUser, game, kepemilikan, riwayat, idPengguna, idGameMasukan, idBeliGame):
  for i in range (my_len(kepemilikan)):
    if idPengguna == kepemilikan[i][1] and idGameMasukan == kepemilikan[i][0]: # 0 untuk game_id dan 1 untuk user_id pada kepemilikan.csv
      ...
      ProsesBeli(dataUser, game, kepemilikan, riwayat, idGameMasukan, idBeliGame)

#PROSES BELI
def ProsesBeli(dataUser, game, kepemilikan, riwayat, idGameMasukan, idBeliGame):
  if dataUser[idPengguna][5] >= game[idBeliGame][4]:
    ...
    riwayat_baru = [idGameMasukan, game[idBeliGame][1], game[idBeliGame][4], idPengguna, 2022]
...
def BeliGame(dataUser, game, kepemilikan, riwayat):
  FormulirBeliGame()
  CariGame(dataUser, game, kepemilikan, riwayat, idPengguna, idGameMasukan)

BeliGame(data, game, kepemilikan, riwayat)